...
- **Observability:** attach a trace listener in development or monitoring if you rely on noticing these failures; they are not written through **`ILogger`** by default. Optionally configure **`ConfigurePulseFlowDiagnostics`** with a **`FlowFault`** callback for structured handling alongside trace.
